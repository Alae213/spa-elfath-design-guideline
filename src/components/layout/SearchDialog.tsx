    ...
    <>
      ...
      {open ? (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-[10vh] p-4">
          <button aria-label="Close search" className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setOpen(false)} />
          <div className="relative w-full max-w-xl rounded-2xl border border-black/10 bg-white shadow-xl dark:border-white/10 dark:bg-[#1c1c1e] overflow-hidden">
            <div className="flex items-center gap-3 border-b border-black/[0.06] px-4 py-3 dark:border-white/[0.06]">
              ...
              />
              <button onClick={() => setOpen(false)} className="h-8 w-8 inline-flex items-center justify-center rounded-full hover:bg-[var(--apple-hover)]" aria-label="Close">
                <CloseIcon size={16} />
              </button>
            </div>
            <div className="max-h-[50vh] overflow-y-auto p-2">
              {query.trim() && results.length === 0 ? (
                <div className="px-4 py-8 text-center text-sm text-[var(--apple-muted)]">No results</div>
              ) : null}
              {!query.trim() ? <div className="px-4 py-6 text-sm text-[var(--apple-muted)]">Type to search</div> : null}
